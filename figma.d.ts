...
interface ViewportAPI {
  ...
  zoom: number;
  scrollAndZoomIntoView(nodes: readonly BaseNode[]);
}
...
interface VectorNetwork {
  ...
  readonly regions?: readonly VectorRegion[]; // Defaults to []
}
...
interface ChildrenMixin {
  readonly children: readonly BaseNode[];

  appendChild(child: BaseNode): void;
  insertChild(index: number, child: BaseNode): void;

  findAll(callback?: (node: BaseNode) => boolean): readonly BaseNode[];
